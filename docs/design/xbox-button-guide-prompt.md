...
- Sources: `vibe_with_xbox/config.py` (`DEFAULT_CONFIG`, `mapping_for_control`) and `vibe_with_xbox/bridge.py` (Start release and RT hold handling).
...
Header at top left, exact text: "Vibe with Xbox" and large bold Chinese title "手柄按键说明". Subtitle: "用 Xbox 手柄操作 Claude Code、macOS 听写与 tmux".
...
1. header "LB · 左肩键"; gesture "单击"; action "上一个窗格"; helper "切换 tmux 焦点". Leader connects exactly to left bumper LB.
...
4. header "Start · 菜单键"; gesture "长按 1 秒后松开"; action "暂停 / 恢复映射"; helper "控制整套按键映射". Leader connects to the small three-line Menu button right of center, NOT to the Xbox logo.
5. neutral note card, no leader, header "暂未映射"; text "LT、右摇杆、摇杆按下"; next text line "View、Share、Xbox 键"; final helper "左摇杆左右方向暂未映射".

RIGHT COLUMN, top to bottom, exact text in cards:
1. A taller two-row shoulder-control card with clear divider. First row: header "RB · 右肩键", gesture "单击", action "下一个窗格". Separate leader to right bumper RB. Second row with pale orange highlight: header "RT · 右扳机", gesture "长按 0.7 秒", action "关闭当前 tmux 窗格". Separate leader to rear right trigger RT; NOT to RB. Tiny orange helper "注意：将终止该窗格".
...
3. Blue X badge header "X 键"; gesture "单击"; action "开启 / 结束听写"; helper "macOS 听写 · 默认 F5". Leader connects exactly to blue X, the left face button.
...
Critical invariants: Simplified Chinese must be correct and readable verbatim. Include all fourteen mapped inputs via the grouped cards. RT is a hold, never single-click. Start toggles only after holding 1 second and releasing. D-pad creates panes, never navigates prompts; left stick up/down navigates. Do not imply Y universally means yes. No claim that unmapped physical controls are disabled system-wide. All leader lines terminate at correct physical controls; route smoothly without crossings where possible.
...
"用 Xbox 手柄操作 Claude Code、macOS 听写与 tmux"
...
The X card immediately above A must have BLUE X badge, header "X 键", "单击" pill, large "开启 / 结束听写", helper "macOS 听写 · 默认 F5", and BLUE X button thumbnail.
...
- File: `docs/assets/xbox-button-guide-zh.png`.
- Visually checked all default control groups and their leader lines, including the right-side Y / B / X / A order.
